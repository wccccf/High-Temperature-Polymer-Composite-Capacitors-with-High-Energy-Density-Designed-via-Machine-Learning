...
class MGD(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.atom_type_input_dim = config.num_atom if 'num_atom' in config else 11
        # contains simple tmb or charge or not qm9:5+1(charge) geom: 16+1(charge)
        self.atom_out_dim = config.num_atom if 'num_atom' in config else 11  # contains charge or not
        self.time_emb = True
        ...
        if self.context is not None:
            if isinstance(self.context, str):
                ctx_nf = 1
                # self.context = [self.context]  # 转换为列表
            else:
    ...
    def forward(self, batch, context=None, return_unreduced_loss=False, return_unreduced_edge_loss=False,
        ...
        atom_noise.normal_()
        atom_type = torch.cat([atom_type[:, :-1] / 4, atom_type[:, -1:] / 16], dim=1)
    ...
    def langevin_dynamics_sample(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, context,
        ...
        print(f"atom_type shape: {atom_type.shape}")  # e.g., [3910, feature_dim]
        print(f"context shape: {context.shape}")  # e.g., [100, context_dim]
        ...
        with torch.no_grad():
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample',total=len(seq)):
