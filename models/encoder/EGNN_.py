...
from models.encoder.EGNN_utils import *
...
class EGNNSparse(MessagePassing):
    ...
    def forward(self, x: Tensor, edge_index: Adj = None,
        ...
        edge_index = edge_index.long()  # 确保 edge_index 是 long 类型
...
class EGNNSparseNetwork(nn.Module):
    ...
    def __init__(self, n_layers, feats_input_dim, feats_dim,
        ...
        for i in range(n_layers):
            ...
            # print(f"is_global_layer:{is_global_layer}")
            if is_global_layer:
    ...
    def forward(self, z, pos, edge_index, edge_attr, batch,
        ...
        """ Recalculate edge features every `self.recalc_edge` with thez
        ...
        for i, layer in enumerate(self.mpnn_layers):
            ...
            if exists(self.global_tokens):
                # print(f"self.global_tokens:{self.global_tokens}")
                # _, amounts = torch.unique(batch, return_counts)
                _, inverse_indices = torch.unique(batch, sorted=False, return_inverse=True)
                amounts = torch.bincount(inverse_indices)
            ...
            if not is_global_layer:
                ...
                print(f"layer[-1]: {type(layer[-1])}")
                print(f"x.shape: {x.shape}, global_tokens.shape: {global_tokens.shape}")
